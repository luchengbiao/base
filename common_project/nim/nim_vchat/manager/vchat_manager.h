...
typedef std::function<void(int volum)> AudioVolumCallback;
...
/** @class VideoManager
  ...
  * @date 2016/09/18
  */
class VChatManager : public nbase::SupportWeakCallback
{
public:
	SINGLETON_DEFINE(VChatManager);
	...
	*/
	bool JoinRoom(nim::NIMVideoChatMode mode, const std::string& room_name, const std::string& rtmp_url, bool live_link, const std::string& session_id, nim::VChat::Opt2Callback cb);

	/**
	* 设置多人音视频回调函数
	* @param[in] connect_cb 连接状态改变的回调函数
	* @param[in] people_cb 成员改变的回调函数
	* @param[in] volume_cb 音量的回调函数
	* @return void	无返回值
	*/
	void SetChatRoomCb(ConnectCallback connect_cb = nullptr, PeopleChangeCallback people_cb = nullptr, VolumeCallback volume_cb = nullptr);

	/**
	* 设置设备检测麦克风音量回调
	* @param[in] audio_cb 麦克风音量回调函数
	*/
	void SetAudioDataCb(AudioVolumCallback audio_cb);

	/**
	* 设备检测麦克风音量回调
	*/
	void OnAudioVolume(uint64_t time, const char *data, uint32_t size, const char *json_extension, const void *user_data);

private:
	int PcmDbCount(const char* ptr, size_t size);
...
private:
	...
	ConnectCallback			chatroom_connect_cb_;
	PeopleChangeCallback	chatroom_people_cb_;
	VolumeCallback			chatroom_volume_cb_;
	AudioVolumCallback      audio_volume_cb_;
};
}