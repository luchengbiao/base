...
void StartChannelCallbackWrapper(int code, const char *session_id, int channel_type, const char* uid, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			...
			{
				channel_id = valus[nim::kNIMRtsChannelId].asString();
			}
			PostTaskToUIHelperThread(std::bind((*cb_pointer), (nim::NIMResCode)code, PCharToString(session_id), channel_type, PCharToString(uid), channel_id));
...
void StartNotifyCallbackWrapper(const char *session_id, int channel_type, const char* uid, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			...
			PostTaskToUIHelperThread(std::bind((*cb_pointer), PCharToString(session_id), channel_type, PCharToString(uid), custom_info, channel_id));
...
void CreateConfCallbackWrapper(int code, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), (nim::NIMResCode)code));
...
void JoinConfCallbackWrapper(int code, const char *session_id, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			...
			PostTaskToUIHelperThread(std::bind((*cb_pointer), (nim::NIMResCode)code, PCharToString(session_id), channel_id, custom_info));
...
void AckCallbackWrapper(int code, const char *session_id, int channel_type, bool accept, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), (nim::NIMResCode)code, PCharToString(session_id), channel_type, accept));
...
void AckNotifyCallbackWrapper(const char *session_id, int channel_type, bool accept, const char* uid, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), PCharToString(session_id), channel_type, accept, PCharToString(uid)));
...
void SyncAckNotifyCallbackWrapper(const char *session_id, int channel_type, bool accept, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), PCharToString(session_id), channel_type, accept, PCharToString(json_extension)));
...
void ConnectNotifyCallbackWrapper(const char *session_id, int channel_type, int code, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), PCharToString(session_id), channel_type, code, PCharToString(json_extension)));
...
void MemberNotifyCallbackWrapper(const char *session_id, int channel_type, int type, const char* uid, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), PCharToString(session_id), channel_type, PCharToString(uid), type));
...
void HangupCallbackWrapper(int code, const char *session_id, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), (nim::NIMResCode)code, PCharToString(session_id)));
...
void HangupNotifyCallbackWrapper(const char *session_id, const char* uid, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), PCharToString(session_id), PCharToString(uid)));
...
void ControlCallbackWrapper(int code, const char *session_id, const char *info, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), (nim::NIMResCode)code, PCharToString(session_id), PCharToString(info)));
...
void ControlNotifyCallbackWrapper(const char *session_id, const char *info, const char* uid, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), PCharToString(session_id), PCharToString(info), PCharToString(uid)));
...
void RecDataCallbackWrapper(const char *session_id, int channel_type, const char* uid, const char* data, unsigned int size, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			...
			PostTaskToUIHelperThread(std::bind((*cb_pointer), PCharToString(session_id), channel_type, PCharToString(uid), data_temp));
