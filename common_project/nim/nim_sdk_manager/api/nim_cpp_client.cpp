...
static void CallbackLogin(const char* json_res, const void *callback)
{
	if (callback != nullptr)
	{
		...
		PostTaskToUIHelperThread(std::bind((*cb), res));
		//(*cb)(res);
	}
}
...
static void CallbackLogout(const char *json_res, const void *callback)
{
	if (callback != nullptr)
	{
		...
		PostTaskToUIHelperThread(std::bind((*cb), error_code));
		//(*cb)(error_code);
		delete cb;
		cb = nullptr;
	}
...
static void CallbackDisconnect(const char* json_res, const void* callback)
{
	if (callback != nullptr)
	{
		...
		PostTaskToUIHelperThread(std::bind((*cb)));
		//(*cb)();
	}
}
	...
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			...
			if (rescode == nim::kNIMResSuccess && reader.parse(PCharToString(content), values) && values.isObject())
			{
				...
				PostTaskToUIHelperThread(std::bind((*cb_pointer), rescode, open));
