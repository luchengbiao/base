	...
	, const void *callback)
{
	if (callback)
	{
		...
		if (*cb_pointer)
		{
			...
			PostTaskToUIHelperThread(std::bind((*cb_pointer), (NIMResCode)res_code, PCharToString(id), to_type, res));
	...
	, const void *callback)
{
	if (callback)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), (NIMResCode)res_code, PCharToString(uid), to_type));
	...
	, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), (NIMResCode)res_code, PCharToString(msg_id)));
	...
	, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), (NIMResCode)res_code));
	...
	, const void *callback)
{
	if (callback)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), imported_count, total_count));
	...
	, const void *callback)
{
	if (callback)
	{
		...
		if (*cb_pointer)
		{
			...
			PostTaskToUIHelperThread(std::bind((*cb_pointer), (nim::NIMResCode)res_code, PCharToString(msg_id), msg_out));
...
static void CallbackMsgStatusChanged(int rescode, const char *result, const char *json_extent, const void *callback)
{
	if (callback)
	{
		...
		if (*cb_pointer)
		{
			...
			PostTaskToUIHelperThread(std::bind((*cb_pointer), result));
//			(*cb_pointer)(result);
		}
	}
}
