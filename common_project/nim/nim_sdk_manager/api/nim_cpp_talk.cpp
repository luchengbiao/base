...
static void CallbackSendMsgAck(const char *result, const void *callback)
{
	if (callback)
	{
		...
		if (*cb_pointer)
		{
			...
			PostTaskToUIHelperThread(std::bind((*cb_pointer), arc));
			//(*cb_pointer)(arc);
		}
	}
}
...
static void CallbackReceiveMsg(const char *content, const char *json_extension, const void *callback)
{
	if (callback)
	{
		...
		if (*cb_pointer)
		{
			...
			PostTaskToUIHelperThread(std::bind((*cb_pointer), msg));
			//(*cb_pointer)(msg);
		}
	}
}
...
static void CallbackReceiveMessages(const char *content, const char *json_extension, const void *callback)
{
	if (callback)
	{
		...
		if (*cb_pointer)
		{
			...
			PostTaskToUIHelperThread(std::bind((*cb_pointer), msgs));
			//(*cb_pointer)(msgs);
		}
	}
}
...
static void CallbackFileUploadProcess(__int64 uploaded_size, __int64 file_size, const char *json_extension, const void *callback)
{
	if (callback)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), uploaded_size, file_size));
	...
	, __int64 timetag/* = 0*/)
{
	...
	values[kNIMMsgKeyLocalTalkId] = receiver_id;
	values[kNIMMsgKeyLocalResId] = client_msg_id;
	values[kNIMMsgKeyLocalLogSubStatus] = kNIMMsgLogSubStatusNotPlaying;
	msg_setting.ToJsonValue(values);
...
static void ReceiveRecallMsg(int rescode, const char *content, const char *json_extension, const void *call_back)
{
	if (call_back)
	{
		...
		if (*cb_pointer)
		{
			...
			PostTaskToUIHelperThread(std::bind((*cb_pointer), kNIMResSuccess, notifys));
		}
	}
}
...
static void CallbackRecallMsg(int rescode, const char *content, const char *json_extension, const void *call_back)
{
	if (call_back)
	{
		...
		if (*cb_pointer)
		{
			...
			PostTaskToUIHelperThread(std::bind((*cb_pointer), (NIMResCode)rescode, notifys));
		}
		delete cb_pointer;
	}
}
