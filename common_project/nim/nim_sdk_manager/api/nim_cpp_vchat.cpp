...
static void mp4_record_opt_cb(bool ret, int code, const char *file, __int64 time, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), ret, code, PCharToString(file), time));
		}
		delete user_data;
	}
}
...
static void OnOptCallback(bool ret, int code, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), ret, code, PCharToString(json_extension)));
		}
		delete user_data;
	}
}
...
static void OnOpt2Callback(int code, __int64 cannel_id, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIHelperThread(std::bind((*cb_pointer), code, cannel_id, PCharToString(json_extension)));
		}
		delete user_data;
	}
}
