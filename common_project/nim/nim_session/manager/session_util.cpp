...
std::wstring GetMessageTime( const long long t, bool abbreviate )
{
	...
	else //mm-dd or yyyy-mm-dd
	{
		if (abbreviate)
		{
			show_data = nbase::StringPrintf(L"%02d/%02d/%02d", msg_time_tm.year(), msg_time_tm.month(), msg_time_tm.day_of_month());
		}
		else
		{
			if (now_time_tm.year() == msg_time_tm.year())
			{
				show_data = nbase::StringPrintf(L"%02d月%02d日", msg_time_tm.month(), msg_time_tm.day_of_month());
			}
			else
			{
				show_data = nbase::StringPrintf(L"%04d年%02d月%02d日", msg_time_tm.year(), msg_time_tm.month(), msg_time_tm.day_of_month());
			}
		}
	}
	...
	{
		if (bToday && msg_time_tm.hour()<12)
		{
			return show_time;
		}
		else if (bToday&&msg_time_tm.hour() > 12)
		{
			return show_time;
		}
		else if (bYesterday)
...
void OpenMap(const std::string& title, const std::string& content, const std::string& point)
{
	...
	if( nbase::ReadFileToString(path_examp, file_data) )
	{
		...
		std::wstring user_data_path = GetUserDataPath(account);
...
bool IsResourceExist(const nim::IMMessage &msg)
{
	...
	if (wpath.empty() || !nbase::FilePathIsExist(wpath, false))
	{
		...
		wpath = GetUserImagePath(account) + filename;
...
void GetNotifyMsg(const std::string& msg_attach, const std::string& from_account, const std::string& to_account, std::wstring &show_text, const std::string& session_id)
{
	...
	if (StringToJson(msg_attach, json))
	{
		...
		std::wstring team_type = TeamManager::GetInstance()->GetTeamType(session_id) == 0 ? L"讨论组" : L"群";
		...
		else
		{
			...
			if (!array.empty() && array.isArray())
			{
				...
				if (!ids.empty())
				{
					...
					std::wstring ttype = TeamManager::GetInstance()->GetTeamType(session_id) == 0 ? L"讨论组" : L"群聊";
