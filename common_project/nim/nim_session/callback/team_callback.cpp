...
{
	LOG_MSG(L"TeamEvent: notify_id={0} tid={1} code={2}") << info.notification_id_ << tid << info.res_code_;
...
void TeamCallback::OnTeamEventCallback(const nim::TeamEvent& result)
{
	...
	LOG_MSG(L"OnTeamEventCallback: notify_id={0} code={1} tid={2}") <<result.notification_id_ <<result.res_code_ <<result.team_id_;
}

void TeamCallback::OnReceiveSysmsgCallback(const nim::SysMessage& msg)
{
	LOG_INFO(L"OnReceiveSysmsgCallback: content: {0} - attach : {1}") << msg.content_ << msg.attach_;
