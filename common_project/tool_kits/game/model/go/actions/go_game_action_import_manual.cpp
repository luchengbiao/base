...
IMPLEMENT_GO_GAME_ACTION_RUNTIME_INFO(GoGameActionType::ImportChessManual, GoActionImportManual)

GameActionResult GoActionImportManual::DoOnBoard(GoGameBoard* game_board)
{
	...
	do
	{
		...
		else
		{
			...
			{
				result.SetErrorInfo(go_actoin_error::CmdGroupIsNullOrEmtpyInManualImported);
				break;
			}
			...
			if (!cmd_new_board)
			{
				result.SetErrorInfo(go_actoin_error::NotExistingNewBoardCmdInManualImported);
				break;
			}
...
void GoActionImportManual::UndoOnBoard(GoGameBoard*)
{}

void GoActionImportManual::RedoOnBoard(GoGameBoard*)
{}
