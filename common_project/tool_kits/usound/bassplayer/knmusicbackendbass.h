...
public:
     ...
     */
    KNMusicBackendBass(std::wstring dllPath);
    ~KNMusicBackendBass();
     ...
     */
	int minimalVolume() const override;

    /*!
     * \brief Reimplemented from KNMusicStandardBackend::maximumVolume().
     */
	int maximumVolume() const override;
...
protected:
     ...
     */
	void setGlobalVolume(const int &volume) override;

    /*!
     * \brief Reimplemented from KNMusicStandardBackend::volumeLevel().
     */
	int volumeLevel() const override;

    /*!
     * \brief Reimplemented from KNMusicStandardBackend::smartVolumeScale().
     */
	double smartVolumeScale() const override;


private:
    inline bool initialBass(DWORD &channelFlags);
    inline void loadPlugin(const std::wstring &dirPath);
    inline KNMusicBackendBassThread *generateThread(const DWORD &channelFlags);
	void GetCurDirFiles(std::wstring path, std::vector<std::wstring>& files);
	inline void  InitPlayer();

    std::list<HPLUGIN> m_pluginList;
    bool m_wasapiEnabled;
	std::wstring s_dll_path_;
	bool b_init_player_;
};
