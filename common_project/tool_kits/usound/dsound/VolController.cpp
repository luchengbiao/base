...
BOOL CVolController::SetMicrPhoneVolume(DWORD dwVolume)
{
	...
	try
	{
		...
		for(int i = 0; i < mixerNum; i ++)
		{
			...
			lineinfo_Connector.dwLineID = lineinfo_Dest.dwLineID;
...
BOOL CVolController::GetMicrPhoneVolume(DWORD &dwVolume)
{
	...
	try
	{
		...
		for(int i = 0; i < mixerNum; i ++)
		{
			...
			//准备获取麦克风设备的ID   
			DWORD dwLineID = 0; 
			//cout << "connections " << dwConnections << endl;
			for(DWORD i = 0; i < dwConnections; i++ )   
			{   
				MIXERLINE MixerLine1;
				//结构体的大小   
				MixerLine1.cbStruct = sizeof(MixerLine1);
				//枚举每一个设备，当Source的ID等于当前的迭代记数   
				MixerLine1.dwSource = i;   
				MixerLine1.dwDestination = MixerLine.dwDestination;
				//根据SourceID获得连接的信息   
				if(mixerGetLineInfo((HMIXEROBJ)m_hMixer, &MixerLine1,   
					MIXER_OBJECTF_HMIXER | MIXER_GETLINEINFOF_SOURCE) != MMSYSERR_NOERROR)
					//判断函数执行错误   
					break;   

				//如果当前设备类型是麦克风，则跳出循环。   
				//cout << "mixer type " << MixerLine1.dwComponentType << endl;
				if(MixerLine.dwComponentType == MIXERLINE_COMPONENTTYPE_SRC_MICROPHONE)   
				{   
					dwLineID = MixerLine.dwLineID;
					dwSource = MixerLine.dwSource;
			...
			//std::cout << "dwLineId " << dwLineID << std::endl;
			//如果没有找到，返回失败。   
			//if(dwLineID == 0)   
			//	continue;    
			...
			if(mixerGetControlDetails(   
				...
				MIXER_GETCONTROLDETAILSF_VALUE)!= MMSYSERR_NOERROR)
				continue;   
			//cout << "dw value " << mxcdVolume.dwValue << " max " << mxc.Bounds.dwMaximum << " min " << mxc.Bounds.dwMinimum <<  endl;
			//cout << "dw value " << mxcdVolume.dwValue << " maxl " << mxc.Bounds.lMaximum << " minl " << mxc.Bounds.lMinimum << endl;
			dwVolume = mxcdVolume.dwValue * 100 / mxc.Bounds.dwMaximum;
			//cout << "dw volmun " << dwVolume << endl;
...
BOOL CVolController::GetMicrophoneLevel()
{
	...
	{	//
		...
		for (int j = 0; j<connections; ++j)
		{
			...
			std::wstring strName = mxc.szName;
			//std::cout << "name " << strName.c_str() << std::endl;
			//strName.MakeUpper();
